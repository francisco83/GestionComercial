...
class Tipos_Cocheras_model extends CI_Model {

	var $table = 'tipos_cocheras';
	...
	{
		//$consulta = $this->db->get($this->table);
		$this->db->select(array('e.id', 'e.nombre', 'e.comentario','e.habilitado'));
		$this->db->from('tipos_cocheras as e');
	...
	public function get_all_export() {
		$this->db->select(array('e.id', 'e.nombre', 'e.comentario'));
		$this->db->from('tipos_cocheras as e');
		$query = $this->db->get();
		return $query->result_array();
	 }
