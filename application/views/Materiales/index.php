			...
			<div class="col-md-12">
				<div class="panel panel-primary">
					<div class="panel-heading">
                        <h4>Materiales</h4>
						<?php if(verifPermiso('IMPRIMIR_ALL', $permisos)){echo'<a class="pull-right btn btn-primary" style="margin-top: -30px" data-toggle="tooltip" title="Imprimir" href="'.site_url().'reportes/materiales" target="_blank"><i class="glyphicon glyphicon-print"></i></a>';}?>					
						<?php if(verifPermiso('EXPORTAR_ALL', $permisos)){echo'<a class="pull-right btn btn-primary" style="margin-top: -30px" data-toggle="tooltip" title="Excel" href="'.site_url().'materiales/createxls"><i class="glyphicon glyphicon-floppy-save"></i></a>';}?>					
					</div>
...
function mostrarDatos(valorBuscar,pagina,cantidad){
	...
	$.ajax({
		...
		success:function(response){		
			filas = "";
			$.each(response.Materiales,function(key,item){
...
function add()
{
    ...
    $('.modal-title').text('Agregar Materiales');
...
// 	//Combo de categorias			 
// 	$.ajax({		
// 		url : "<?php echo site_url('Categorias_Materiales/get_all_array');?>",
// 		type: "POST",
// 		dataType:"json",
// 		success:function(response){
...
function edit(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('#modal_form').modal('show');
            $('.modal-title').text('Editar Materiales');
			$('.modal-backdrop').remove();
...
// 	//Recupero los pagos			 
// 	$.ajax({
// 		url : "<?php echo site_url('Materiales/verdetallehistorico')?>",
// 		type: "POST",
// 		dataType:"json",
...
<div class="modal" id="modal_historico" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Historico de Precios de Materiales</h3>
            </div>
